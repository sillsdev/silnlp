...
def download_project(SF_id: str):
    ...
    try:
        ...
        os.makedirs("/root/OnboardingProjects", exist_ok=True)
        with open(f"/root/OnboardingProjects/{SF_id}.zip", "wb") as f:
...
for request in onboarding_requests:
    for SF_id in get_SF_ids(request):
        download_project(SF_id)
# TODO: Distinguish between projects and resources

if os.path.exists("/root/onboarded_projects_log.txt"):
    with open("/root/onboarded_projects_log.txt", "r") as f:
...
def process_request(request):
    ...
    subprocess.run(["python", "/workspaces/silnlp/scripts/automate_onboard_project.py", "--task-name", task_name])
    # Get a clearml task by its name
    task: Task = Task.get_task(project_name="Onboarding", task_name=task_name)
    with open("/root/onboarded_projects_log.txt", "a") as f:
