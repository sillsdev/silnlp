...
class _S3ConfigurationMap:
    ...
    def __repr__(self):
        ...
        return "{name}(arguments={arguments}, resources={resources})".format(
            name=type(self).__name__, arguments=self.arguments, resources=self.resources
        )
...
class _S3Scandir:
    ...
    def __iter__(self):
        ...
        kwargs = {"Bucket": bucket.name, "Prefix": self._s3_accessor.generate_prefix(self._path), "Delimiter": sep}
...
class _S3Accessor(_Accessor):
    ...
    def open(self, path, *, mode="r", buffering=-1, encoding=None, errors=None, newline=None):
        ...
        return file_object(
            object_summery, path=path, mode=mode, buffering=buffering, encoding=encoding, errors=errors, newline=newline
        )
    ...
    def owner(self, path):
        ...
        responce = object_summery.meta.client.list_objects_v2(
            Bucket=object_summery.bucket_name, Prefix=object_summery.key, FetchOwner=True
        )
    ...
    def rename(self, path, target):
        ...
        if not self.is_dir(path):
            ...
            self.boto3_method_with_parameters(target_bucket.copy, args=(old_source, target_key_name))
        ...
        for object_summery in bucket.objects.filter(Prefix=source_key_name):
            ...
            self.boto3_method_with_parameters(target_bucket.copy, args=(old_source, new_key))
            self.boto3_method_with_parameters(object_summery.delete)
    ...
    def boto3_method_with_parameters(self, boto3_method, config=None, args=(), kwargs=None):
        ...
        if config is not None:
            kwargs.update(
                {key: value for key, value in config.items() if key in self._get_action_arguments(boto3_method)}
            )
    ...
    def unlink(self, path, *args, **kwargs):
        ...
        try:
            self.boto3_method_with_parameters(
                bucket.meta.client.delete_object, config=config, kwargs={"Bucket": bucket_name, "Key": key_name}
            )
        except ClientError:
    ...
    def _get_action_arguments(self, action):
        if isinstance(action.__doc__, LazyLoadedDocstring):
            ...
            docs = action.__doc__
        return set(
            line.replace(":param ", "").strip().strip(":") for line in docs.splitlines() if line.startswith(":param ")
        )
...
class S3Path(_PathNotSupportedMixin, Path, PureS3Path):
    ...
    def open(self, mode="r", buffering=DEFAULT_BUFFER_SIZE, encoding=None, errors=None, newline=None):
        ...
        return self._accessor.open(
            self, mode=mode, buffering=buffering, encoding=encoding, errors=errors, newline=newline
        )
...
class S3KeyWritableFileObject(RawIOBase):
    def __init__(
        self, object_summery, *, path, mode="w", buffering=DEFAULT_BUFFER_SIZE, encoding=None, errors=None, newline=None
    ):
        super().__init__()
    ...
    def write(self, text):
        ...
        self._cache.seek(0)
        _s3_accessor.boto3_method_with_parameters(self.object_summery.put, kwargs={"Body": self._cache})
...
class S3KeyReadableFileObject(RawIOBase):
    def __init__(
        self, object_summery, *, path, mode="b", buffering=DEFAULT_BUFFER_SIZE, encoding=None, errors=None, newline=None
    ):
        super().__init__()
        self.object_summery = object_summery
        self.path = path
    ...
    def readable_check(method):
        ...
        def wrapper(self, *args, **kwargs):
            if not self.readable():
                raise UnsupportedOperation("not readable: " + str(self.path))
            return method(self, *args, **kwargs)
    ...
    def _line_iterator(self):
        if not self._streaming_line_iterator:
            self._streaming_line_iterator = self._streaming_body.iter_lines(chunk_size=self.buffering)
...
class S3DirEntry:
    ...
    def __repr__(self):
        return "{}(name={}, is_dir={}, stat={})".format(type(self).__name__, self.name, self._is_dir, self._stat)
