...
def main() -> None:
    ...
    parser.add_argument("--files", help="Comma-delimited list of patterns of extract file names to count (e.g. 'arb-*.txt;de-NT.txt)",required=True)
    ...
    for file in args.files.split(';'):
        ...
        for extract_file_name in tqdm(extract_files_list):
            with open(SIL_NLP_ENV.assets_dir / "vref.txt", 'r', encoding='utf-8') as vref_file, \
                ...
                verse_counts.append({'file': os.path.basename(extract_file_name), 'per_book_counts': Counter(book_list), 'per_chapter_counts': chapter_counts})
    ...
    for filename, books in partially_complete_books.items():
        df = pd.DataFrame(columns=[i for i in range(1,max([len(complete_book_counts[book].keys()) for book in books]))])
    ...
    verse_percentage_df.insert(loc=0, column='Total', value=verse_percentage_df[OT_canon + NT_canon + DT_canon].mean(axis=1).round(1))
