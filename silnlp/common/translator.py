        ...
        for draft_index, translated_draft in enumerate(translated_text_rows.get_translated_drafts(), 1):
            ...
            for config in postprocess_handler.configs:
                ...
                for chapter_num in chapters_for_remarks:
                    draft_remark = (
                        f"This draft of {sentences.get_book()} {chapter_num} was generated by AI from {draft_src_str} on "
                        f"{date.today()} using model {experiment_ckpt_str}. It should be "
                        f"reviewed and edited carefully."
                    )
                    if postprocess_remark:
                        draft_remark += " " + postprocess_remark
