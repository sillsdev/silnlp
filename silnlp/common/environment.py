...
class SilNlpEnv:
    ...
    def copy_pt_project_from_bucket(self, name: Union[str, Path], patterns: Union[str, Sequence[str]] = []):
        if not self.is_bucket:
            return
        name = str(name)
        pt_projects_path = self.data_dir / "Paratext" / "projects" if os.environ.get('SIL_NLP_PT_DIR', None) is not None else str(self.pt_dir.relative_to(self.data_dir) / "projects") + "/"
