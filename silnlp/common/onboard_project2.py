    ...
    if args.copy_from:
        ...
        for project in args.projects:
            ...
            if normalized_name != project:
                ...
                else: project_folder = normalized_folder
            if args.datestamp:
        ...
        if not args.no_clean:
            ...
            old_argv = sys.argv
            try:
                sys.argv = ["clean_projects", "--folders"] + [str(project_folder) for project_folder in project_folders]
                clean_projects.main()
            finally:
                sys.argv = old_argv
        ...
        for project_folder in project_folders:
            ...
            LOGGER.info(f"Copying {project_name} from {copy_from_path} to {destination_folder}")
            copy_paratext_project_folder(project_folder, project_name, overwrite=args.overwrite)

            if args.extract_corpora:
                LOGGER.info(f"Extracting corpora from {project_name}")
                extract_config = config.get("extract_corpora", {})
                extract_corpora(projects={project_name}, books_to_include=extract_config.get("include", []), books_to_exclude=extract_config.get("exclude", []), include_markers=extract_config.get("markers", False), extract_lemmas=extract_config.get("lemmas", False), extract_project_vrefs=extract_config.get("project-vrefs", False))

            if args.collect_verse_counts:
                if not args.extract_corpora: LOGGER.warning("--extract_corpora was not included. Collecting verse counts requires the corpus to be extracted first.")
                LOGGER.info(f"Collecting verse counts from {project_name}.")
                collect_verse_counts_wrapper(project_name, config.get("verse_counts", {}))

            if args.wildebeest:
                if not args.extract_corpora: LOGGER.warning("--extract_corpora was not included. Wildebeest requires the corpus to be extracted first.")
                ...
                old_argv = sys.argv
                try:
                    sys.argv = ["wb_ana", "-i", extract_file, "-j", f"{project_name}_wildebeest.json", "-o", f"{project_name}_wildebeest.txt", "-x", str(wildebeest_config.get("max_examples", 500)), "-n", str(wildebeest_config.get("max_cases", 500)), "-r", str(wildebeest_config.get("ref_id_file", "silnlp/assets/vref.txt"))]
                    wb_ana.main()
                finally:
                    sys.argv = old_argv

            if args.stats:
                if not args.extract_corpora: LOGGER.warning("--extract_corpora was not included. Stats requires the corpus to be extracted first.")
        ...
        for project in args.projects:
            normalized_name = project.replace('-', '_')
            if normalized_name != project: LOGGER.info(f"Using normalized name: {normalized_name} for project {project}")
            destination_folder = SIL_NLP_ENV.pt_projects_dir / normalized_name
            if not destination_folder.exists(): errors.append(f"Project not found in destination: {destination_folder}")
            project_names.append(normalized_name)
        ...
        if not args.no_clean:
            LOGGER.info("Cleaning Paratext project folders.")
            project_folders = [SIL_NLP_ENV.pt_projects_dir / project_name for project_name in project_names]
            old_argv = sys.argv
            try:
                sys.argv = ["clean_projects", "--folders"] + [str(project_folder) for project_folder in project_folders]
                clean_projects.main()
