...
from .book_args import add_books_argument, expand_book_list, get_epilog, get_sfm_files_to_process
...
def find_break_positions(text):
    ...
    while i < len(text):
        if text[i] in SENTENCE_ENDS:
            j = i + 1
            while j < len(text) and text[j] in CLOSE_QUOTES:
                j += 1
...
def split_text(text, max_len):
    ...
    while start < len(text):
        ...
        if best is None:
            best = start + max_len  # hard cut as last resort

        chunks.append(text[start:best])
        start = best
        while start < len(text) and text[start] in WORD_BREAKS:
            start += 1
...
def split_text_token(parser, max_len):
    ...
    for chunk in chunks[1:]:
        for elem in stack:
            ...
            if token_type:
                result.append(UsfmToken(token_type, marker=elem.marker))
            else:
                print(f"Warning: unknown element type in stack: {elem.type}")
        result.append(UsfmToken(UsfmTokenType.TEXT, text=chunk))
    return result
...
def main():
    parser = argparse.ArgumentParser(
        description="Split long paragraphs in USFM files",
    ...
    if not args.books:
        books = expand_book_list(VALID_BOOKS)
        sfm_files = get_sfm_files_to_process(project_dir, books)
        print(f"No books specified, will process these sfm files: {[file.name for file in sfm_files]}")
    else:
        books = expand_book_list(args.books)
        sfm_files = get_sfm_files_to_process(project_dir, books)
        print(f"Books specified: {args.books}.\nWill process these sfm files: {[file.name for file in sfm_files]}")
    ...
    for sfm_file in sfm_files:
        ...
        with open(output_sfm_file, "w", encoding="utf-8") as f:
            f.write(usfm_stripped)
