#Copied from translator.py and main from translate.py
import string
...
class Translator(ABC):
    ...
    def check_book(
        ...
        book_path = get_book_path(src_project, book)
        stylesheet = get_stylesheet(src_project_dir)
        print(f"Found book {book} in source project {src_project}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Check that we can find the books to translate")
    ...
    if len(args.books) > 0:
        if args.debug:
            ...
            exit()
        translator.check_books(args.books, args.src_project, args.trg_iso, args.include_inline_elements)
    else:
        raise RuntimeError("At least one book must be specified.")
