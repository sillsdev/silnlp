...
def main() -> None:
    ...
    parser.add_argument(
        "--stats", help="True or False: Print word count and number of renderings for common words", action="store_true"
    )
    ...
    lex_path = environment.align_experiments_dir / args.experiment
    ...
    if args.stats:
        ...
        print(
            f"The {args.num} most common source words in the dataset have an average of "
            f"{round((common_wd_renderings)/args.num)} target renderings per word."
        )
        print(
            f"The {args.num} most diverse source words in the dataset have an average of "
    ...
    with (lex_path / "lex_stats.csv").open("w", encoding="utf8") as stats_file:
        ...
        writer.writerow(
            [
                len(src_words),
                len(trg_words),
                len(unique_src_words),
                len(unique_trg_words),
                args.num,
                round((common_wd_instances) / args.num),
                round((common_wd_renderings) / args.num),
