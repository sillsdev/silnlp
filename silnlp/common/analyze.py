...
class CorpusAnalysis:
    """Alignment scores and verse counts for the corpora of one experiment."""
    ...
    def run(
        self, exp_name: str, recalculate: bool = False, deutero: bool = False, create_summaries: bool = False
    ) -> None:
        ...
        collect_verse_counts(
            self._environment.mt_scripture_dir,
        ...
        self.corpus_stats(exp_name, recalculate, deutero)

        # Add stats about projects in extra alignment files in the experiment folder
        extra_projects = self.extra_alignments(deutero)
        ...
        if create_summaries:
            try:
                self.alignment_breakdown_file(deutero)
                self.summary_file()
            except Exception as e:
                LOGGER.error(f"Error creating summary files: {e}")

    def corpus_stats(self, exp_name: str, force_align: bool = False, deutero: bool = False) -> None:
        stats_path = self._files.corpus_stats()
        ...
        for pair in self._corpus_pairs:
            for src_file, trg_file in get_data_file_pairs(pair):
                ...
                if force_align:
                    (self._files.pair_scores(src_project, trg_project)).unlink(missing_ok=True)
                    (self._files.pair_scores(trg_project, src_project)).unlink(missing_ok=True)
        ...
        for src_file, trg_file, corpus_books, score_threshold in pairs_to_process:
            ...
            if project_pair not in stats_df.index:
                ...
                corpus = get_scripture_parallel_corpus(src_file.path, trg_file.path, False, environment=self._environment)
                if not deutero:
                ...
                # Align pair
                pair_stats_path = self._files.pair_scores(project_pair[0], project_pair[1])
                alt_pair_stats_path = self._files.pair_scores(project_pair[1], project_pair[0])
                ...
                src_script_in_model = is_represented(src_script, self._model_name)
                trg_script_in_model = is_represented(trg_script, self._model_name)
    ...
    def extra_alignments(self, deutero: bool = False) -> List[str]:
        stats_path = self._files.corpus_stats()
        ...
        skipped = set()
        extra_projects = []
        for filepath in self._files.score_files():
            ...
            if project_pair not in stats_df.index and project_pair not in stats_df.swaplevel().index:
                ...
                src_path = self._environment.get_mt_corpus_path(project_pair[0])
                if src_path.is_file():
                    extra_projects.append(src_path.name)
                trg_path = self._environment.get_mt_corpus_path(project_pair[1])
                if trg_path.is_file():
                    extra_projects.append(trg_path.name)

                if src_path.is_file() and trg_path.is_file():
                    corpus = get_scripture_parallel_corpus(src_path, trg_path, False, environment=self._environment)
                    if not deutero:
                        corpus = corpus.loc[[is_ot_nt(vref.book_num) for vref in corpus["vref"]]]
                    pair_count = len(corpus.index)
                    src_only_count = len(corpus.loc[corpus["target"].str.len() == 0].index)
    ...
    def summary_file(self) -> None:
        corpus_stats_df = pd.read_csv(self._files.corpus_stats(), index_col=["src_project", "trg_project"])
        verse_counts_df = pd.read_csv(self._files.verse_counts(), index_col="file")
        verse_percentages_df = pd.read_csv(self._files.verse_percentages(), index_col="file")
        alignment_scores_df = pd.read_excel(
            self._files.alignment_breakdown(),
            sheet_name="By Book",
            index_col=[0, 1],
        )
        ...
        # Write to Excel file
        with pd.ExcelWriter(self._files.analysis(), engine="xlsxwriter") as writer:
    ...
    def alignment_breakdown_file(self, deutero: bool) -> None:
        ...
        for project_pair in tqdm(corpus_stats_df.sort_index().index):
            ...
            scores_path = self._files.pair_scores(src_project, trg_project)
            ...
            if not scores_path.is_file():
                scores_path = self._files.pair_scores(trg_project, src_project)
        ...
        with pd.ExcelWriter(self._files.alignment_breakdown()) as writer:
...
def analyze(
    ...
    CorpusAnalysis(
        AnalysisFiles(config.exp_dir),
        config.corpus_pairs,
        config.data["aligner"],
        config.model,
        environment,
    ).run(exp_name, recalculate, deutero, create_summaries)
