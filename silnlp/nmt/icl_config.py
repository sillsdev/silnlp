...
This is a third implementation of the :class:`Config`/:class:`NMTModel` abstractions,
alongside the seq2seq (NLLB/MADLAD) implementation in :mod:`silnlp.nmt.seq2seq_config` and the
fine-tuned decoder-only implementation in :mod:`silnlp.nmt.llm_config`. Unlike both of those,
it does **no fine-tuning**: it translates by prompting a hosted LLM with parallel examples
drawn from the training corpus. Calls go through LiteLLM, so one ``model`` string selects any
supported provider (Anthropic, OpenAI, Gemini, Bedrock, a local server, ...).

Like the fine-tuned LLM implementation, it reuses the model-agnostic parts of the pipeline by
setting ``data.tokenize: false``, so preprocessing writes raw detokenized parallel text and
evaluation (:mod:`silnlp.nmt.test`) and inference orchestration (:mod:`silnlp.nmt.translate`)
work unchanged.
...
* **Batch granularity** (``infer.batch.granularity``): ``verse`` translates ``infer.batch.size``
  segments per request; ``chapter`` translates a whole chapter per request.
...
returns them (OpenAI and Azure do, Anthropic never does) and one segment per request; see
``ICLModel._check_confidences_supported``.
"""
...
GRANULARITY_VERSE = "verse"
GRANULARITY_CHAPTER = "chapter"
VALID_GRANULARITIES = (GRANULARITY_VERSE, GRANULARITY_CHAPTER)

# The train step writes this checkpoint so that CheckpointType.LAST resolves to step 1.
CHECKPOINT_STEP = 1
MODEL_INFO_FILENAME = "icl_model.json"
...
# Only used to warn about oversized full-corpus prompts; an exact count would need a
# provider-specific tokenizer.
_CHARS_PER_TOKEN = 4

_CODE_FENCE = re.compile(r"^\s*```[^\n]*\n(.*?)\n?\s*```\s*$", re.DOTALL)
_NUMBERED_LINE = re.compile(r"^\s*(\d{1,4})\s*[.):\]]\s*(.*)$")
_VREF = re.compile(r"^(\S+)\s+(\d+):")
...
def group_indices_by_chapter(vrefs: Sequence[str], max_size: int) -> List[List[int]]:
    """Group line indices into contiguous runs that share a book and chapter.

    Lines with no parsable reference - blank lines for non-scripture rows, and ``<range>``
    markers - continue the current group rather than starting a new one. Groups are capped at
    ``max_size`` so that a long chapter does not produce an unmanageable request.
    """
    max_size = max(1, max_size)
    groups: List[List[int]] = []
    current_key: Optional[Tuple[str, str]] = None
    for index, vref in enumerate(vrefs):
        match = _VREF.match(vref.strip())
        key = (match.group(1), match.group(2)) if match is not None else None
        starts_group = len(groups) == 0 or (key is not None and key != current_key) or len(groups[-1]) >= max_size
        if starts_group:
            groups.append([])
...
@dataclass(frozen=True)
class Completion:
    """A reply from the model, with its per-token log probabilities when they are available."""
...
class CompletionClient(ABC):
    ...
    Indirected so tests can substitute a scripted client without a network call (mirrors
    CausalLMProvider in llm_config.py).
    """
...
class LiteLLMCompletionClient(CompletionClient):
    ...
    def complete(self, messages: List[Dict[str, str]], logprobs: bool = False) -> Completion:
        ...
        response = litellm.completion(
            ...
            max_tokens=self._infer["max_tokens"],
        ...
        return Completion(content if content is not None else "", extract_token_logprobs(choice) if logprobs else [])
...
def _import_litellm():
    try:
        ...
        raise ImportError(
            "In-context learning experiments require the 'litellm' package, which is part of the "
            "'icl' extra. Install it with `poetry install -E icl`."
        ) from e
    return litellm


class CompletionClientFactory:
    def create(self, config: "ICLConfig") -> CompletionClient:
...
class ICLConfig(Config):
        ...
        config = merge_dict(
            {
                ...
                "infer": {
                    ...
                    "retrieval": {
                        ...
                        # better but needs rank_bm25 from the 'icl' extra.
                        "method": TFIDF_METHOD,
                        "num_examples": 10,
                    },
                    "batch": {
                        "granularity": GRANULARITY_VERSE,
                        "size": 1,
                        "max_size": 60,
                    },
                    "num_drafts": 1,
    ...
    def _validate(self) -> None:
        ...
        if self.batch_granularity not in VALID_GRANULARITIES:
            raise ValueError(
                f"Unknown infer.batch.granularity '{self.batch['granularity']}'. "
                f"Valid options: {', '.join(VALID_GRANULARITIES)}."
            )
        for name, value in (
            ("infer.batch.size", self.batch["size"]),
            ("infer.batch.max_size", self.batch["max_size"]),
    ...
    @property
    def batch_granularity(self) -> str:
        return str(self.batch["granularity"]).lower()
    ...
    def render_examples(self, examples: Sequence[ExamplePair], src_lang: Language, trg_lang: Language) -> str:
        template: str = self.prompt["example_template"] or DEFAULT_EXAMPLE_TEMPLATE
        return "\n\n".join(
            template.format(
    ...
    def build_system_message(self, src_lang: Language, trg_lang: Language, corpus_block: Optional[str] = None) -> str:
        system_message: str = self.prompt["system_message"] or DEFAULT_SYSTEM_MESSAGE_TEMPLATE.format(
            src_lang=src_lang.name, trg_lang=trg_lang.name
        )
        if corpus_block:
    ...
    def build_user_message(
        ...
        if len(sources) == 1:
            template: str = self.prompt["instruction_template"] or DEFAULT_SINGLE_INSTRUCTION_TEMPLATE
            ...
            batch_template: str = self.prompt["batch_instruction_template"] or DEFAULT_BATCH_INSTRUCTION_TEMPLATE
            parts.append(
    ...
    def build_messages(
        ...
        """
        return [
            {"role": "system", "content": self.build_system_message(src_lang, trg_lang, corpus_block)},
            {"role": "user", "content": self.build_user_message(sources, examples, src_lang, trg_lang)},
        ]
    ...
    def create_model(
        ...
        return ICLModel(self, completion_client_factory)
...
class ICLModel(NMTModel):
    ...
    def train(self) -> None:
        ...
        if self._config.context_mode == CONTEXT_MODE_RAG:
            ...
            estimated_tokens = self._estimate_corpus_tokens(pairs)
            info["estimated_corpus_tokens"] = estimated_tokens
            LOGGER.info(
                "Full-corpus mode: %d training examples, roughly %d tokens, will be included in every request.",
                len(pairs),
                estimated_tokens,
            )
            self._warn_if_corpus_too_large(estimated_tokens)
    ...
    def _estimate_corpus_tokens(self, pairs: Sequence[ExamplePair]) -> int:
        characters = sum(len(pair.source) + len(pair.target) for pair in pairs)
        return characters // _CHARS_PER_TOKEN

    def _warn_if_corpus_too_large(self, estimated_tokens: int) -> None:
        limit: int = self._config.infer["max_context_tokens"]
        if estimated_tokens > limit:
            LOGGER.warning(
                "The training corpus is roughly %d tokens, which exceeds infer.max_context_tokens (%d). "
    ...
    def _get_corpus_block(self, src_lang: Language, trg_lang: Language) -> Optional[str]:
        ...
        with self._lock:
            if self._corpus_block is None:
                pairs = self._load_training_pairs()
                self._warn_if_corpus_too_large(self._estimate_corpus_tokens(pairs))
                rendered = self._config.render_examples(pairs, src_lang, trg_lang)
    ...
    def translate(
        ...
        batches = self._batch_indices(len(sentence_list), vrefs)
    ...
    def translate_test_files(
        ...
        for input_path, translation_path in zip(input_paths, translation_paths):
            ...
            batches = self._batch_indices(len(sentences), self._read_vrefs(input_path, len(sentences)))
            groups = list(
    ...
    def _check_confidences_supported(self) -> None:
        ...
        caught here.
        """
        if self._config.batch_granularity != GRANULARITY_VERSE or self._config.batch["size"] != 1:
            raise RuntimeError(
                ...
                "to individual segments. Set infer.batch.granularity to "
                f"'{GRANULARITY_VERSE}' and infer.batch.size to 1, or run without --save-confidences."
            )
        if not self._get_client().supports_logprobs():
            raise RuntimeError(
                ...
                "Use a model that supports logprobs, such as an OpenAI or Azure OpenAI model, or "
                "run without --save-confidences."
            )
    ...
    def _read_vrefs(self, input_path: Path, expected_count: int) -> Optional[List[str]]:
        """Read the verse references that line up with a test source file, if they are available."""
        suffix = ".src.txt"
        if not input_path.name.endswith(suffix):
            return None
        vref_path = input_path.with_name(input_path.name[: -len(suffix)] + ".vref.txt")
        if not vref_path.is_file():
            return None
        vrefs = _read_lines(vref_path)
        if len(vrefs) != expected_count:
            LOGGER.warning(
                "%s has %d lines but %s has %d; ignoring the verse references.",
                vref_path.name,
                len(vrefs),
                input_path.name,
                expected_count,
    ...
    def _batch_indices(self, count: int, vrefs: Optional[Sequence[str]]) -> List[List[int]]:
        size: int = self._config.batch["size"]
        if self._config.batch_granularity == GRANULARITY_CHAPTER:
            if vrefs is not None and len(vrefs) == count:
                return group_indices_by_chapter(vrefs, self._config.batch["max_size"])
            LOGGER.warning(
                "Chapter batching needs verse references, which are not available for this input; "
                "falling back to batches of %d segments.",
                size,
            )
        return group_indices_by_size(count, size)
    ...
    def _translate_batches(
        ...
        def run_task(task: Tuple[int, int]) -> None:
            ...
            completions = self._translate_batch([sentences[i] for i in indices], src_lang, trg_lang, want_logprobs)
    ...
    def _translate_batch(
        self, texts: Sequence[str], src_lang: Language, trg_lang: Language, want_logprobs: bool = False
    ) -> List[Completion]:
        ...
        if len(non_blank) == 0:
            return translations
        completed = self._complete_texts([text for _, text in non_blank], src_lang, trg_lang, want_logprobs)
    ...
    def _complete_texts(
        ...
        """
        if len(texts) == 1:
            return [self._complete_single(texts[0], src_lang, trg_lang, want_logprobs)]

        messages = self._build_messages(texts, src_lang, trg_lang)
        response = self._get_client().complete(messages).text
        ...
        if parsed is None:
            ...
            parsed = parse_numbered_response(self._get_client().complete(correction).text, len(texts))
        ...
        middle = len(texts) // 2
        return self._complete_texts(texts[:middle], src_lang, trg_lang, want_logprobs) + self._complete_texts(
            texts[middle:], src_lang, trg_lang, want_logprobs
        )

    def _complete_single(
        self, text: str, src_lang: Language, trg_lang: Language, want_logprobs: bool = False
    ) -> Completion:
        completion = self._get_client().complete(self._build_messages([text], src_lang, trg_lang), want_logprobs)
