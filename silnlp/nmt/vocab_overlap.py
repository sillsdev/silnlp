...
def main() -> None:
    ...
    print(
        f"\tSize:\tExperiment 1: {len(exp1_src_vocab)}\tExperiment 2: {len(exp2_src_vocab)}\tTotal: {len(all_src_vocab)}"
    )
    print(f"\tOverlap:\tExperiment 2 vs Experiment 1: {len(overlap_src_vocab)}")
    print("Target vocabulary metrics:")
    print(
        f"\tSize:\tExperiment 1: {len(exp1_trg_vocab)}\tExperiment 2: {len(exp2_trg_vocab)}\tTotal: {len(all_trg_vocab)}"
    )
