...
def get_names_files(
    src_files: List[LangsDataFile], trg_langs: Set[str]
) -> Tuple[List[LangsDataFile], List[LangsDataFile]]:
    src_names_files: List[LangsDataFile] = []
    trg_names_files: List[LangsDataFile] = []
        ...
        if os.path.isfile(src_names_path):
            for trg_iso in trg_langs:
                ...
                if os.path.isfile(trg_names_path):
                    src_names_files.append(LangsDataFile(src_names_path, DataFileType.TRAIN))
                    trg_names_files.append(LangsDataFile(trg_names_path, DataFileType.TRAIN))
...
class LangsConfig(Config):
    def __init__(self, exp_dir: str, config: dict) -> None:
        ...
        self.src_names_files, self.trg_names_files = get_names_files(self.src_files, trg_isos)
        src_file_paths: Set[str] = set(df.path for df in itertools.chain(self.src_files, self.src_names_files))
        trg_file_paths: Set[str] = set(df.path for df in itertools.chain(self.trg_files, self.trg_names_files))
    ...
    def _build_corpora(self, stats: bool) -> None:
        ...
        for src_names_file, trg_names_file in zip(self.src_names_files, self.trg_names_files):
            ...
            if self.mirror:
                ...
                self._insert_trg_tag(src_names_file.iso, mirror_cur_names)
                names = pd.concat([names, cur_names], ignore_index=True)

            self._insert_trg_tag(trg_names_file.iso, cur_names)
