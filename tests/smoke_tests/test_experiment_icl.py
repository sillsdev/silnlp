...
from silnlp.nmt.icl_config import ICLConfig
...
EXPERIMENT_NAME = "test_experiment_icl"


def test_icl_experiment_full_pipeline():
...
def create_experiment_with_mock_client(environment: SilNlpEnv) -> Tuple[SILExperiment, CompletionStats]:
    ...
    assert isinstance(config, ICLConfig)

    # An ICL model takes a completion client factory rather than a pretrained model provider, so
