from silnlp.nmt.nllb_demo import pages
from silnlp.nmt.nllb_demo.scoring_service import (
    _FlagCandidate,
...
_LANGS = ["eng_Latn", "fra_Latn", "spa_Latn"]


def test_suggest_page_renders_shared_nav_and_config():
    html = pages.suggest_page(_LANGS, "eng_Latn", "fra_Latn")
    assert "<title>Suggest · NLLB Demo</title>" in html
    # Shared nav links to both demos, with Suggest marked active.
    assert '<a href="/suggest" class="active">Suggest</a>' in html
    assert '<a href="/evaluate">Evaluate</a>' in html
    # Languages/defaults are injected via the single config blob.
    assert '"defaultSrc": "eng_Latn"' in html
    assert '"defaultTgt": "fra_Latn"' in html
    assert "fra_Latn" in html
    # Suggest-specific UI is present.
    assert 'id="confidenceThreshold"' in html
    assert "/api/suggest" in html


def test_evaluate_page_renders_shared_nav_and_config():
    html = pages.evaluate_page(_LANGS, "fra_Latn", "eng_Latn")
    assert "<title>Evaluate · NLLB Demo</title>" in html
    assert '<a href="/evaluate" class="active">Evaluate</a>' in html
    assert '<a href="/suggest">Suggest</a>' in html
    assert '"defaultSrc": "fra_Latn"' in html
    # Evaluate-specific UI is present.
    assert 'id="target-editor"' in html
