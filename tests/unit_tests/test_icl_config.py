...
from silnlp.nmt.config_utils import is_icl_config, is_llm_config
...
from silnlp.nmt.icl_config import (
...
def test_is_icl_config_requires_an_explicit_model_type():
    assert is_icl_config({"model_type": "icl", "model": "anthropic/claude-sonnet-4-5"})
    assert is_icl_config({"model_type": "ICL", "model": "gpt-4o"})
    # A LiteLLM model string is arbitrary, so it is never recognized on its own.
    assert not is_icl_config({"model": "anthropic/claude-sonnet-4-5"})
    assert not is_icl_config({})


def test_an_icl_config_is_not_claimed_by_the_llm_dispatch():
    # google/gemma matches LLM_MODEL_PREFIXES, but the explicit model_type has to win.
    config = {"model_type": "icl", "model": "google/gemma-2-2b-it"}
    assert is_icl_config(config)
...
def test_group_indices_by_chapter_splits_on_book_and_chapter():
    vrefs = ["GEN 1:1", "GEN 1:2", "GEN 2:1", "EXO 1:1"]
    assert group_indices_by_chapter(vrefs, 60) == [[0, 1], [2], [3]]


def test_group_indices_by_chapter_keeps_unreferenced_lines_with_the_current_chapter():
    # Blank lines mark non-scripture rows and "<range>" marks a merged verse; neither starts a
    # new chapter.
    vrefs = ["GEN 1:1", "", "<range>", "GEN 2:1"]
    assert group_indices_by_chapter(vrefs, 60) == [[0, 1, 2], [3]]


def test_group_indices_by_chapter_caps_group_size():
    vrefs = [f"PSA 119:{verse}" for verse in range(1, 6)]
    assert group_indices_by_chapter(vrefs, 2) == [[0, 1], [2, 3], [4]]
...
def make_config(exp_dir: Path, **overrides) -> ICLConfig:
    config: dict = {
        "model_type": "icl",
        "model": "gpt-4o",
    ...
    for key, value in overrides.items():
        ...
        if isinstance(section, dict) and isinstance(value, dict):
            ...
            config[key] = value
    return ICLConfig(exp_dir, config, Mock())
...
def test_config_defaults(tmp_path: Path):
    ...
    assert config.batch_granularity == "verse"
    assert config.batch["size"] == 1
...
def test_config_requires_a_model(tmp_path: Path):
    with pytest.raises(ValueError, match="LiteLLM format"):
        ICLConfig(tmp_path, {"model_type": "icl", "model": "", "data": {"corpus_pairs": []}}, Mock())
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"batch": {"granularity": "book"}}, "Unknown infer.batch.granularity"),
        ({"batch": {"size": 0}}, "infer.batch.size"),
...
def make_model(
    ...
    token_logprobs: Optional[List[TokenLogprob]] = None,
    **overrides,
) -> Tuple[ICLModel, ScriptedClient]:
    client = ScriptedClient(responder, logprobs_supported, token_logprobs)
    config = make_config(tmp_path, **overrides)
    return ICLModel(config, ScriptedClientFactory(client)), client
...
def test_translate_batches_several_segments_into_one_request(tmp_path: Path):
    model, client = make_model(tmp_path, echo_translations, infer={"batch": {"size": 3}, "concurrency": 1})
    groups = list(model.translate(["one", "two", "three"], "en", "es"))
...
def test_translate_leaves_blank_segments_blank_without_a_request(tmp_path: Path):
    model, client = make_model(tmp_path, echo_translations, infer={"batch": {"size": 4}, "concurrency": 1})
...
def test_translate_keeps_blank_segments_aligned_within_a_batch(tmp_path: Path):
    model, _ = make_model(tmp_path, echo_translations, infer={"batch": {"size": 3}, "concurrency": 1})
...
def test_translate_falls_back_to_single_requests_when_the_reply_is_malformed(tmp_path: Path):
    ...
    model, client = make_model(
        tmp_path, lambda messages: "I translated everything for you!", infer={"batch": {"size": 2}, "concurrency": 1}
    )
...
def test_translate_recovers_after_a_corrective_retry(tmp_path: Path):
    ...
    model, client = make_model(tmp_path, lambda messages: next(replies), infer={"batch": {"size": 2}, "concurrency": 1})
    groups = list(model.translate(["one", "two"], "en", "es"))
...
def test_concurrent_requests_keep_the_translations_in_order(tmp_path: Path):
    model, _ = make_model(tmp_path, echo_translations, infer={"batch": {"size": 1}, "concurrency": 4})
...
def test_translate_test_files_batches_by_chapter_using_the_vref_file(tmp_path: Path):
    (tmp_path / "test.src.txt").write_text("a\nb\nc\n", encoding="utf-8")
    (tmp_path / "test.vref.txt").write_text("GEN 1:1\nGEN 1:2\nGEN 2:1\n", encoding="utf-8")
    model, client = make_model(
        tmp_path, echo_translations, infer={"batch": {"granularity": GRANULARITY_CHAPTER}, "concurrency": 1}
    )

    model.translate_test_files([tmp_path / "test.src.txt"], [tmp_path / "out.txt"])

    # Two chapters means two requests, even though batch.size is the default of 1. The second
    # chapter has a single verse, so it uses the unnumbered single-segment prompt.
    assert len(client.calls) == 2
...
def test_chapter_batching_falls_back_when_there_are_no_verse_references(tmp_path: Path):
    model, client = make_model(
        tmp_path,
        echo_translations,
        infer={"batch": {"granularity": GRANULARITY_CHAPTER, "size": 2}, "concurrency": 1},
    )
    groups = list(model.translate(["a", "b", "c"], "en", "es"))
...
# --- verse references from the USFM path --------------------------------------------------


USFM = r"""\id 3JN
\c 1
\s A heading
\v 1 The elder to the beloved Gaius.
\v 2 Beloved, I pray that all may go well with you.
\c 2
\v 1 I had much to write to you.
"""


def make_usfm_row_collection(tmp_path: Path):
    from machine.corpora import UsfmFileText, UsfmStylesheet

    from silnlp.common.translation_data_structures import UsfmTextRowCollection

    path = tmp_path / "653JN.SFM"
    path.write_text(USFM, encoding="utf-8-sig")
    stylesheet = UsfmStylesheet("usfm.sty")
    file_text = UsfmFileText(stylesheet, "utf-8-sig", "3JN", path, include_all_text=True)
    return UsfmTextRowCollection(file_text, "en", stylesheet)


def test_usfm_refs_are_line_parallel_with_the_sentences(tmp_path: Path):
    # The two lists have to stay aligned, or chapter grouping would be applied to the wrong rows.
    rows = make_usfm_row_collection(tmp_path)
    sentences = rows.get_sentences_for_translation()
    refs = rows.get_refs_for_translation()

    assert len(refs) == len(sentences)
    assert all(ref.startswith("3JN ") for ref in refs)
    # The heading belongs to chapter 1 along with its verses, and chapter 2 follows.
    assert [ref.split(":")[0] for ref in refs] == ["3JN 1", "3JN 1", "3JN 1", "3JN 2"]


def test_usfm_refs_group_into_chapters(tmp_path: Path):
    rows = make_usfm_row_collection(tmp_path)
    groups = group_indices_by_chapter(rows.get_refs_for_translation(), 60)
    assert groups == [[0, 1, 2], [3]]


def test_translate_batches_by_chapter_when_refs_are_supplied(tmp_path: Path):
    # This is the drafting path: translate_usfm passes the refs it collected from the rows.
    model, client = make_model(
        tmp_path, echo_translations, infer={"batch": {"granularity": GRANULARITY_CHAPTER}, "concurrency": 1}
    )
    groups = list(model.translate(["a", "b", "c"], "en", "es", vrefs=["GEN 1:1", "GEN 1:2", "GEN 2:1"]))

    assert len(client.calls) == 2
    assert translations_of(groups) == ["translated 1", "translated 2", "translated"]


def test_translate_ignores_refs_that_do_not_line_up(tmp_path: Path):
    model, client = make_model(
        tmp_path, echo_translations, infer={"batch": {"granularity": GRANULARITY_CHAPTER}, "concurrency": 1}
    )
    groups = list(model.translate(["a", "b", "c"], "en", "es", vrefs=["GEN 1:1"]))

    assert len(groups) == 3
    # Falls back to size batching (the default size of 1) rather than mis-grouping.
    assert len(client.calls) == 3
...
def test_confidences_are_rejected_in_chapter_mode(tmp_path: Path):
    model, client = make_model(
        ...
        logprobs_supported=True,
        token_logprobs=SCORES,
        infer={"batch": {"granularity": GRANULARITY_CHAPTER}},
    )
