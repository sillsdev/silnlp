...
class Status(Enum):
    ...
    def from_clowder_investigation(i: investigation.Investigation):
        experiments = list(i.experiments.items()) #TODO helper method
        ...
        if len(align_exps) > 0 and len(list(filter(lambda exp: exp[1]['status'] == 'completed', align_exps))) == len(align_exps):
            return Status.Aligned
        stats_exp = list(filter(lambda exp: 'stats' in exp[0],experiments))
        if len(stats_exp) > 0 and len(list(filter(lambda exp: exp[1]['status'] == 'completed', stats_exp))) == len(stats_exp):
...
class Investigation:
    ...
    def from_clowder(clowder_investigation):
        return Investigation(id=clowder_investigation.id, name=clowder_investigation.name,status=Status.from_clowder_investigation(clowder_investigation)) #clowder_investigation.status)
